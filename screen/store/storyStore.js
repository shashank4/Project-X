...
// Check both the condition as described above
  ...
  var handleCharaOfDelete = function(aParent, iIndex, aGrandParent, iGrandIndex, iRange){
    ...
    else if (aParent[iIndex].XMLElement)
    {
      ...
      if(firstContBr.Br){
        var fID = firstContBr.Br[0]['$']['data-uid'];
        _setCaretPositionAccordingToObjectDelete(fID);
  ...
  var handleCharaOfBackSpace = function (aCustom, charIndex) {

    var iClone = charIndex;


    /**  characterStyleRange handling*/
    /**if aCustom is having more than 0 child elements i.e. more than one characterStyleRanges. */
    if (aCustom.length > 0) {
      ...
      if (aCustom[charIndex].CharacterStyleRange[0].Custom[lastElementIndex - 1].Br) {
        ...
        if (lastElementIndex != 1) {
          aCustom[charIndex].CharacterStyleRange[0].Custom.splice(lastElementIndex-1, 1);    ///zzz
        ...
        /** if last Br is the ONLY child*/
          ...
          /** if aCustom has more than 1 element , i.e. AaCustom has more charaStyles or xml tags in siblings*/
      ...
      /**if last element of prev chara is Content*/
      ...
      /**if last element of prev chara is  XMLElement */
  ...
  var handleXMLOfBackSpace = function(aParent, i, aChild, iChild){
    ...
    if(aParent[i].XMLElement[0].Custom[lastIndex-1].Br){
      ...
      if(aParent[i].XMLElement[0].Custom.length == 0){                            /** *********************************************************************************  */
        aParent[i].XMLElement[0].Custom.push(_createContentNode(""));
        _setCaretPositionAccordingToObject(aParent[i].XMLElement[0].Custom[0]);
      }


      /*if(aParent[i+1].Content && aParent[i+1].Content[0]["_"].length == 0){
        aParent.splice(i+1);
      }*/
    }
    ...
    /** if last node is Content */
      ...
      if (str.length > 1) {
        ...
        if(aParent[i].XMLElement[0].Custom.length > 0){
          ...
          oNode = _getLastChildNode(aParent[i-1]);
          _setCaretPositionAccordingToObject(oNode);
        }
  ...
  var _setCaretPosition = function (aParent, iReturnedObjectIndex, aPath, currentStory) {
    ...
    if (!_.isEmpty(oPreviousNode)) {
      ...
      var aGrandParent = oSearchedObject.objectPos;
      var iIndex = oSearchedObject.indexPos;
      if (iIndex > 0) {
        oNodeToSetCaret = _getLastChildNode(aGrandParent[iIndex - 1]);
        _setCaretPositionAccordingToObject(oNodeToSetCaret);

      } else {
        _setCaretPosition(aGrandParent, iIndex + 1, aPath, currentStory);
      }
    }
  };
  ...
  var _handleMiddleAll = function(oRange, sStyleId, oCurrentStory, oStartDOM, oEndDOM, startBrFlag, endBrFlag){
    ...
    var j=1;
    var aPathLength=aPaths.length-1;
    /*if(startBrFlag){
      j=0;
    }else{
      j=1;
  ...
  var _handleCharaStyleInSameContent = function(startOffset, endOffset, selfPath, sStyleID, oCurrentStory, isMultiSel, isForward){
    ...
    if(aParent[iParent].XMLElement){
      ...
      var currentContentOrBr = aSelf.splice(iSelf);
      ...
      if(middleChara){
        ...
        var BrCharacterStyle = _createCharacterStyleRange(sStyleID, currentContentOrBr );
        aSelf.push(BrCharacterStyle);
      }
    ...
    /** if CONTENT's parent is CharacterStyleRange , then create 3 different charaStyles*/
      ...
      if(middleNewChara){
        ...
        var newBrCharacterStyle = _createCharacterStyleRange(sStyleID, aCurrentSelf);
        aParent.push(newBrCharacterStyle);
      }
      ...
      if(!isMultiSel && middleNewCharaFirstChild){
  ...
  var _applySelectedStyleToSelectedNode = function (oRange, sStyleType, sStyleId) {
    ...
    if(sStyleType == "Character Styles") {
      _applyCharacterStyleToSelectedNode(oRange, sStyleId, oCurrentStory, aStartDOMPath, aEndDOMPath);
    }
  ...
  var _setCaretXMLPreviousNode = function(aGrandParent, iGrandIndex, pathForChara, currentStory){
    aGrandParent.splice(iGrandIndex, 1);
    if(aGrandParent.length != 0 ){
      ...
      var iGrandIndex2 = oGrandParent2.indexPos;

      _setCaretXMLPreviousNode(aGrandParent2, iGrandIndex2, pathForChara, currentStory);
  ...
  var handleContentBackSpace = function(aParent, iParent, iStartIndex, targetPath, path, currentStory, currentStoryId, iRangeOffset, oCurrentDom, selFocusOffset){
    ...
    if (iStartIndex == 0 || iRangeOffset == 0 || selFocusOffset==0) {
      ...
      if (iParent != 0) {
        ...
        /** if prev node is characterSrtleRange*/
          ...
          iParentClone = handleCharaOfBackSpace(aParent, iParent - 1);
          iParentClone += iParentClone;

        }
        /** if prev node is paragraphStyleRange*/
        else if (aParent[iParent - 1].ParagraphStyleRange){
          //TODO: create this function.   (Shashank)
          //handleParaOfBackSpace(aParent, iParent - 1);
        }
     ...
     */
    else if (iStartIndex ==1 && aParent[iParent].Content[0]['_'].length == 1) {
      if (aParent.length == 1) {
        ...
        if (aGrandParent[iGrandIndex].CharacterStyleRange || oCurrentDom.parentNode.className.indexOf("characterContainer") > (-1)) {
          ...
          if (aGrandParent.length > 1) {
            ...
            if (aGrandParent[iGrandIndex + 1] && aGrandParent[iGrandIndex + 1].CharacterStyleRange
              ...
              if(caretFocusFlag == 0){
                oCaretPosition.indexToFocus  = aGrandParent[iGrandIndex - 1].CharacterStyleRange[0].Custom[last].Content[0]["_"].length;
              }
    ...
    /** for normal backSpace. */
      ...
      var str = aParent[iParent].Content[0]["_"];
  ...
  var handleBrBackSpace = function(aParent, iParent, iStartIndex, targetPath, path, currentStory, currentStoryId, iRangeOffset, oCurrentDom){
    ...
    if (aParent[iParent - 1] ) {
      ...
      if(aParent[iParent - 1].Br){
        ...
        if(tempStr.length == 0 ){
          ...
          {
            handleContentBackSpace(aParent, iParent-1, (tempStr.length), targetPath, path, currentStory, currentStoryId, iRangeOffset, oCurrentDom);
          }
        }
        else
        {
  ...
  var _getGrandParent = function(currentStory, targetPath){
    var path = targetPath.split('/');
    path.splice(0,1);
    path.splice(-1,1);
    path.splice(-1,1);

    return _searchClosestCustomOfLastInPath(currentStory, path);
  ...
  var storyStore =  {
    ...
    handleBackspacePressed: function (oEvent, oCurrentDom, iStartIndex, targetPath, oSelection, iRangeOffset) {
      ...
      if(sType == "Caret") {
        ...
        if (aParent[iParent].Br) {
          ...
          handleBrBackSpace(aParent, iParent, iStartIndex, targetPath, path, currentStory, currentStoryId, iRangeOffset, oCurrentDom);
    ...
    handleDeletePressed: function (oEvent, sel, targetPath) {
      ...
      if(selType == "Caret"){
        ...
        if(aParent[iIndex].Br && returnedObject.flag==true){

          /** handle empty content nodes.*/
          if(aParent[iIndex+1] && aParent[iIndex+1].Content && aParent[iIndex+1].Content[0]['_'].length==0){
            aParent.splice(iIndex+1, 1);
          }
        ...
        /** if key is pressed on Content node */
        else if(aParent[iIndex].Content){
          /** if key is pressed on the last position of the current node */
          if (iRange.endOffset == aParent[iIndex].Content[0]["_"].length)    //sel is necessary.
          {
